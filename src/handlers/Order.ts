...
import { DepositETH, OrderExecuted, OrderCancelled, TraxCore } from '../entities/TraxCore/TraxCore'
...
/**
 * @dev ERC20 transfer should have an extra data we use to identify a traxfolio order.
 * A transfer with a traxfolio order looks like:
 *
 * 0xa9059cbb
 ...
 *
 * The important part is 2070696e652e66696e616e6365 which is the hexa of ` traxfolio` used for the secret.
...
export function handleOrderCreationByERC20Transfer(event: Transfer): void {
  ...
  let traxCore = TraxCore.bind(getAddressByNetwork(dataSource.network()))
...
export function handleOrderCancelled(event: OrderCancelled): void {
  ...
  let traxCore = TraxCore.bind(event.address)
  let res = traxCore.try_existOrder(
